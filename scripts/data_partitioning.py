...
def split_station(
    ...
    shuffle: bool = True,
    seed: int = None,
    stratify: xr.DataArray = None,
):

    if stratify is not None:
        stratify_list = stratify.values.tolist()
        stratify_list = cluster_single_class(stratify_list)

    train_val, test = ms.train_test_split(
        station_array.values.tolist(),
        test_size=p[-1],
        shuffle=shuffle,
        random_state=seed,
    )
    ...
    if stratify is not None:
        stratify_list = stratify.loc[train_val].values.tolist()
        stratify_list = cluster_single_class(stratify)

    if stratify:
        cv = ms.StratifiedShuffleSplit(n_splits, train_size=p_train, random_state=seed)
        split_kwargs = {"X": train_val, "y": stratify}
    else:
        cv = ms.ShuffleSplit(n_splits, train_size=p[0] / (1 - p[-1]), random_state=seed)
        split_kwargs = {"X": train_val}

    splits = []
    for train, val in cv.split(**split_kwargs):
        split = {
            "train": sorted(np.array(train).tolist()),
            "val": sorted(np.array(val).tolist()),
            "test": sorted(test),
        }
        splits.append(split)
    return splits
...
def main(inputs, outputs, config):
    ...
    if isinstance(dp_config.get("station", None), dict):
        station_splits = split_station(station_array, **dp_config["station"], seed=seed)
    else:
        station_splits = [None]
